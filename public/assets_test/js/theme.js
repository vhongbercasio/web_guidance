...
var appCalendarInit = function appCalendarInit() {
  ...
  if (appCalendar) {
    $.ajax({
      url: scheduleClassroomEventsUrl,
      method: 'GET',
      dataType: 'json',
      success: function (data) {
        ...
        var calendar = renderCalendar(appCalendar, {
          ...
          dateClick: function (dateClick) {
            var modal = new window.bootstrap.Modal(addEventModal);
            modal.show();
            startDateInput.value = dateClick.dateStr; 
            var flatpickr = document.querySelector(Selectors.EVENT_START_DATE)._flatpickr;
            flatpickr.setDate([dateClick.dateStr]);
          }
        });
        ...
        document.querySelectorAll(Selectors.DATA_CALENDAR_VIEW).forEach(function (link) {
          link.addEventListener(Events.CLICK, function (e) {
            e.preventDefault();
            var el = e.currentTarget;
            var viewType = el.dataset.fcView;
  ...
  addEventModal && addEventModal.addEventListener('shown.bs.modal', function (e) {
      ...
      success: function (data) {
        ...
        $.ajax({
          ...
          success: function (attendanceData) {
            // Store attendance dates in a Set for quick lookup
            var attendanceDates = new Set(attendanceData);

            var calendar = new FullCalendar.Calendar(appCalendar, {
              initialView: 'dayGridMonth',
              headerToolbar: false,
              dayMaxEvents: 2,
              height: 1000,
              ...
              dayCellDidMount: function (info) {
                ...
                let localDateString = localDate.toISOString().split('T')[0];


                let isHoliday = events.some(event => event.start === localDateString && event.is_holiday == 1);

                if (isHoliday) {
                  info.el.style.backgroundColor = '#C75B7A'; 
                  var dayNumberEl = info.el.querySelector('.fc-daygrid-day-number');
                  if (dayNumberEl) {
                    dayNumberEl.style.backgroundColor = '#FCDE70'; 
                  }
                } else if (attendanceDates.has(localDateString)) {
                  info.el.style.backgroundColor = '#A1D6B2'; 
                  var dayNumberEl = info.el.querySelector('.fc-daygrid-day-number');
                  if (dayNumberEl) {
                    dayNumberEl.style.backgroundColor = '#FCDE70'; 
                  }
...
document.addEventListener('DOMContentLoaded', appCalendarInit);
